...
def test_outage_language_automatically_drives_diagnosis_and_sandbox_recovery(db):
    ...
    assert cycle["reason"] == "automatic_outage_trigger"
    assert [tool["tool"] for tool in cycle["tool_calls"]] == [
        "check_service_health", "get_error_rate", "inspect_service_logs",
        "get_recent_deployments",
    ]
    assert "pricing servlet" in cycle["diagnosis"].lower()
    assert cycle["remediation"]["status"] == "SIMULATED_EXECUTED"
    ...
    assert same_cycle["timestamp"] == cycle["timestamp"]
    assert db.query(EventLog).filter_by(event_type="REMEDIATION_EXECUTED").count() == 1
